...
PGStore.prototype.set = function (sid, sessData, callback) {
  this.getClient(function (client) {
    ...
    client.query(
       'select web.set_session_data($1, $2, $3)'
  ...
  this.getClient(function (client) {
    client.query(
      ...
      ,function (err, result) {
         ...
         if (result) {
           if (result.rowCount) {
             if(typeof(result.rows[0].get_session_data)=='string'){
               callback(null, JSON.parse(result.rows[0].get_session_data));
             }else{
               callback(null, result.rows[0].get_session_data);
             }
           } else {
             callback(null, null);
           }
